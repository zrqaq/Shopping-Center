			...
			<div class="search_view">
				...
				<div class="search_box">
					<el-select
						class="search_sel"
						clearable
						v-model="searchQuery.xingbie" 
						placeholder="性别"
						>
						<el-option v-for="item in xingbieLists" :label="item" :value="item"></el-option>
					</el-select>
				</div>
			</div>
			<div class="search_view">
					...
					<div class="data_item" v-for="(item,index) in list" :key="index" @click.stop="detailClick(item.id)" >
						...
						<div class="data_content">
							<div class="data_title1">
								<span>{{item.shangjiazhanghao}}</span>
							</div>
						</div>
					</div>
				</div>
...
<script setup>
	...
	const getList = () => {
		...
		if(searchQuery.value.xingbie&&searchQuery.value.xingbie!=''){
			params.xingbie = searchQuery.value.xingbie
		}
